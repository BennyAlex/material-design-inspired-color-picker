var mdColorPicker = (function () {
	...
	{
	  module.exports = vue_runtime_common_dev;
	}
	});
	...
	const colorIsLight = function colorIsLight(color, differenceRange) {
	  if (color[0] === '#') {
	...
	const colorIsDark = function colorIsDark(color, differenceRange) {
	...
	var fullMaterialPalette = Object.keys(materialPalette).reduce(function (palette, key) {
	  palette[key] = materialPalette[key];

	  if (accentMaterialPalette[key]) {
	    palette[key] = _extends({}, palette[key], accentMaterialPalette[key]);
	  }
	...
	var script = {
	  ...
	  created() {
	    ...
	    Object.keys(this.currentPalette).forEach(function (key) {
	      let valueOrObject = _this2.currentPalette[key];
	...
	var __vue_render__ = function() {
	  ...
	  return _c(
	    ...
	    {
	      staticClass: "mdcp",
	      style: _vm.fixedMinHeight
	        ? { width: _vm.wrapperWidth, minHeight: _vm.wrapperMinHeight }
	    ...
	    [
	      _vm.subPalette !== undefined
	        ? _c(
	            "div",
	            {
	              staticClass: "mdcp-back-icon",
	              style: {
	                order: _vm.backButtonAtEnd ? 2 : 1,
	                margin: _vm.colorMargin + "px",
	                height: _vm.colorSizePx,
	                width: _vm.colorSizePx
	              },
	              on: {
	            ...
	            [
	              _c(
	                ...
	                {
	                  attrs: {
	                    fill: _vm.backButtonColor,
	                    height: _vm.colorSize,
	                    width: _vm.colorSize / 2,
	                    viewBox: "0 0 24 24",
	      ...
	      _vm._l(_vm.colors, function(color) {
	        return _c(
	          ...
	          {
	            key: color.name,
	            staticClass: "mdcp-color",
	            class: {
	              "mdcp-selected":
	                color.value.toLowerCase() === _vm.value.toLowerCase() ||
	                _vm.isTintOfSelected(color),
	              "mdcp-is-light": _vm.colorIsLight(color.value)
	            },
	            style: _vm.getColorStyle(color),
	            attrs: { title: color.name },
	            on: {
	              click: function($event) {
	                $event.stopPropagation();
	                return _vm.click(color)
	              }
	            }
	          },
	          [_c("span", { staticClass: "mdcp-circle", style: _vm.circleStyle })]
	        )
	      })
	    ],
	    2
	...
	vue_runtime_common.use(vueCustomElement);
	vue_runtime_common.customElement('md-color-picker', __vue_component__); // this is the color picker class which can be accessed from outside
	...
	ColorPicker.colorIsDark = colorIsDark; // make the colorPicker class accessible from outside
